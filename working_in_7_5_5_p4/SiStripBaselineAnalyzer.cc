...
SiStripBaselineAnalyzer::SiStripBaselineAnalyzer(const edm::ParameterSet& conf){
  ...
  h1ClusterSkew_ = fs_->make<TH1I>("ClusterSkewness","Cluster Skewness;Cluster Skewness;nCluster", 100, -20, 20); 
}
		...
		for ( edmNew::DetSet<SiStripCluster>::const_iterator clus =	itClusters->begin(); clus != itClusters->end(); ++clus){
		  if(itClusters->id() == detId){
		    ...
		    for( auto itAmpl = clus->amplitudes().begin(); itAmpl != clus->amplitudes().end(); ++itAmpl){
		      ...
		      skew += strip2*strip2*strip2*(*itAmpl);
		    }
                    ...
                    skew = (((skew/charge-3*sigma*sigma*mean-mean*mean*mean)<0)?-1:1)*TMath::Power(TMath::Abs(skew/charge-3*sigma*sigma*mean-mean*mean*mean),0.33);
                    h1ClusterSkew_->Fill(skew);
                    if(nclust%10000==0) std::cout << strip2 << " "<<charge << " " <<mean<<" "<<sigma<<" "<<skew<<std::endl;
                  }              
		}
	  }
        if(ClusterDists==false)
